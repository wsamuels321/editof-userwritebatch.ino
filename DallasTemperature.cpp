...
//---------added getFieldandAddr() and setFieldandAddr
...
// note if device is not connected it will fail writing the data.
void DallasTemperature::setFieldandAddr(const uint8_t* deviceAddress, uint8_t za[2]) {
...
uint8_t * DallasTemperature::getFieldandAddr(const uint8_t* deviceAddress) {
	static uint8_t data[2];
	ScratchPad scratchPad;
	if (isConnected(deviceAddress, scratchPad)) {
		data[0] = scratchPad[HIGH_ALARM_TEMP] << 8;
		data[1] = scratchPad[LOW_ALARM_TEMP];
	}
	return data;
}
